...
public protocol TagsViewLayoutDelegate: class {
    ...
    func tagCellLayoutTagSize(layout: TagsViewLayout, atIndex index:Int) -> CGSize
    func tagCellLayoutInteritemHorizontalSpacing(layout: TagsViewLayout) -> CGFloat
    func tagCellLayoutInteritemVerticalSpacing(layout: TagsViewLayout) -> CGFloat
    
}


public class TagsViewLayout: NSCollectionViewFlowLayout {
        ...
        public init(alignment: LayoutAlignment = .left, delegate: TagsViewLayoutDelegate?) {
        ...
        public override func layoutAttributesForItem(at indexPath: IndexPath) -> NSCollectionViewLayoutAttributes {
            guard layoutInfoList.count > indexPath.item else { return NSCollectionViewLayoutAttributes.init() }
            return layoutInfoList[indexPath.item].layoutAttribute
        }
        
        override public func layoutAttributesForElements(in rect: CGRect) -> [NSCollectionViewLayoutAttributes] {
            guard !layoutInfoList.isEmpty else { return [] }
...
private extension TagsViewLayout {
    
    var currentTagFrame: CGRect {
        guard let info = currentTagLayoutInfo?.layoutAttribute else { return .zero }
        var frame = info.frame
        frame.origin.x += info.size.width + (delegate?.tagCellLayoutInteritemHorizontalSpacing(layout: self) ?? 0.0)
        return frame
    }
    ...
    func layoutAttribute(tagIndex: Int, tagFrame: CGRect) -> NSCollectionViewLayoutAttributes {
        let indexPath = IndexPath(item: tagIndex, section: 0)
        let layoutAttribute = NSCollectionViewLayoutAttributes(forItemWith: indexPath)
...
extension TagsViewLayout {
       ...
       struct LayoutInfo {
           ...
           var layoutAttribute: NSCollectionViewLayoutAttributes
           var whiteSpace: CGFloat = 0.0
           var isFirstElementInARow = false
           
           init(layoutAttribute: NSCollectionViewLayoutAttributes) {
