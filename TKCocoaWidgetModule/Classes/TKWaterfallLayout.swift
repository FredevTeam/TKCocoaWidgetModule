//
//  TKWaterfallLayout.swift
//  TKCocoaWidgetModule
//
//  Created by 抓猫的鱼 on 2020/8/5.
//

import Foundation


import Cocoa

/// 瀑布流代理
@objc public protocol WaterfallLayoutDataSource : class {
    
    /// 指定ITEM的高度
    ///
    /// - Parameters:
    ///   - layout: 布局
    ///   - indexPath: 位置
    /// - Returns: 高度
    func waterfallLayout(_ layout : WaterfallLayout,itemHeight indexPath : IndexPath) -> CGFloat
    
    /// 瀑布流一共有多少列，默认时三列
    /// - Parameter layout: 布局
    /// - Returns: 列数
    @objc optional func numberOfColsInWaterfallLayout(_ layout : WaterfallLayout) -> Int
}

public class WaterfallLayout: NSCollectionViewFlowLayout {
    
    // MARK: 对外提供属性
    // 瀑布流数据源代理
    public weak var dataSource : WaterfallLayoutDataSource?
    
    // MARK: 私有属性
    // 布局属性数组
    private lazy var attrsArray : [NSCollectionViewLayoutAttributes] = [NSCollectionViewLayoutAttributes]()
    
    //每一列的高度累计
    private var colHeights : [CGFloat] = []
    
    // 最高的高度
    private var maxH : CGFloat = 0
    
    //索引
    private var startIndex = 0
    
    override init() {
        super.init()
        self.initColheights()
    }
    
    required init?(coder: NSCoder) {
        fatalError("init(coder:) has not been implemented")
    }
    
}

extension WaterfallLayout {
    func reset() {
        self.initColheights()
        self.attrsArray.removeAll()
        self.maxH = 0
        self.startIndex = 0
    }
    private func initColheights() {
        //列数
        let cols = self.dataSource?.numberOfColsInWaterfallLayout?(self) ?? 2
        //根据列数创建数组
        self.colHeights = Array(repeating: self.sectionInset.top, count: cols)
    }
}


extension WaterfallLayout {
    
    public override func prepare() {
        super.prepare()
        
        //获取item的个数
        let itemCount = collectionView!.numberOfItems(inSection: 0)
        
        //获取列数
        let cols = dataSource?.numberOfColsInWaterfallLayout?(self) ?? 2
        
        //计算Item的宽度（屏幕宽度铺满）
        let itemW = (collectionView!.bounds.width - self.sectionInset.left - self.sectionInset.right - self.minimumInteritemSpacing * CGFloat(cols - 1)) / CGFloat(cols)
        
        //计算所有的item的属性
        for i in startIndex..<itemCount {
            // 设置每一个Item位置相关的属性
            let indexPath = IndexPath(item: i, section: 0)
            
            // 根据位置创建Attributes属性
            let attrs = NSCollectionViewLayoutAttributes.init(forItemWith: indexPath)
            
            // 获取CELL的高度
            guard let height = dataSource?.waterfallLayout(self, itemHeight: indexPath) else {
                fatalError("请设置数据源,并且实现对应的数据源方法")
            }
            
            //取出当前列所属的列索引
            let index = i % cols
            
            //获取当前列的总高度
            var colH = colHeights[index]
            
            //将当前列的高度在加载当前ITEM的高度
            colH = colH + height + minimumLineSpacing
            
            //重新设置当前列的高度
            colHeights[i % cols] = colH
            
            // 5.设置item的属性
            attrs.frame = CGRect(x: self.sectionInset.left + (self.minimumInteritemSpacing + itemW) * CGFloat(index), y: colH - height - self.minimumLineSpacing, width: itemW, height: height)
            
            attrsArray.append(attrs)
        }
        
        // 4.记录最大值
        maxH = colHeights.max() ?? 0
        
        // 5.给startIndex重新复制
        startIndex = itemCount
    }
}

extension WaterfallLayout {
    public override func layoutAttributesForElements(in rect: CGRect) -> [NSCollectionViewLayoutAttributes] {
        return attrsArray
    }
    
    public override var collectionViewContentSize: CGSize {
        return CGSize(width: 0, height: maxH + sectionInset.bottom - minimumLineSpacing)
    }
}
